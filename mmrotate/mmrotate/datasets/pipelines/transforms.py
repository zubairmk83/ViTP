...
from mmdet.datasets.pipelines import (Mosaic, RandomCrop,
                                                 RandomFlip, Resize,Collect)
...
class MultiBranch(object):
    def __init__(self, branch_field,
                 **branch_pipelines) -> None:
    ...
    def __call__(self, results: dict) -> dict:
        ...
        """
        # assert False, results # dict_keys(['img_metas', 'img', 'gt_bboxes', 'gt_labels'])
        res_dict = results.keys()
        format_results = {}
        for key in res_dict:
            format_results[key] = {}
        for branch in self.branch_field:    
            for key in res_dict:
                format_results[key][branch] = None
                if branch in self.branch_pipelines:
                    if isinstance(results[key], list):
                        format_results[key][branch] = results[key]
                    else:
                        format_results[key][branch] = results[key].data
        for key in res_dict:
            # cpu_only = key=='img_metas'
            format_results[key] = DC(format_results[key], cpu_only=True)
                
        # assert False, format_results
...
class RResize(Resize):
    ...
    def __init__(self,
        ...
        self.rotate_bboxes = rotate_bboxes
    def _resize_bboxes(self, results):
        """Resize bounding boxes with ``results['scale_factor']``."""
        for key in results.get('bbox_fields', []):
            bboxes = results[key]
            orig_shape = bboxes.shape
            if self.rotate_bboxes:
