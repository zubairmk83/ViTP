...
class TIMMVisionTransformer(nn.Module):
    ...
    """
    def __init__(self, img_size=224, pretrain_size=224, patch_size=16, in_chans=3, num_classes=1000,
    ...
    def init_weights(self, pretrained=None):
        def resize_pos_embed(pos_embed, H, W):
            ...
            pos_embed = pos_embed[:, 1:, :].reshape(
                1, self.pretrain_size // 16, self.pretrain_size // 16, -1).permute(0, 3, 1, 2)
        ...
        if isinstance(pretrained, str):
            ...
            for key in checkpoint:
                new_key = key.replace('.ls1.gamma','.gamma1')
                new_key = new_key.replace('.ls2.gamma','.gamma2')
