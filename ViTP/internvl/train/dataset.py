...
def smart_concat(string1, string2):
    """
    拼接两个字符串，如果第一个字符串的结尾是空格或换行符，则直接拼接第二个字符串；
    否则，在两个字符串之间添加一个空格。
    """
...
def preprocess_internvl2_5(
    ...
    for conversation in conversations:
        if conversation['from'] == 'human':
            ...
            roles.append('human')
            if '<cls>' in conversation['value']:
                q = conversation['value'].split(': ')
                if len(q)<2:
                    continue
                ques = q[1].split('.')[0]
                choices = ques.split('|')
        elif conversation['from'] == 'gpt':
            gpt_value = conversation["value"]
            choices.append(gpt_value)
            if use_CoT and 'reason' in conversation:
                assert False
                gpt_value = smart_concat(conversation["reason"], conversation["value"])
                # assert False, (f'<|im_start|>assistant\n{gpt_value}<|im_end|>\n')
    ...
    if len(choices) > 0:
        ...
        answer_tokens = tokenizer(gpt_value, return_tensors='np').input_ids[0][0]
        choices_ids = tokenizer(
            choices,
            return_tensors='np',
            padding=False,
            max_length=tokenizer.model_max_length,
        ...
        # turn choices_ids into a list
        choices_ids = list(set([item[0] for item in choices_ids]))
        # print(f'answer_tokens: {answer_tokens}')
        # print(f'choices_ids: {choices_ids}')
        assert answer_tokens in choices_ids, (f'{answer_tokens} not in {choices_ids}')
    if add_bos_token:  # for InternLM series
    ...
    for role, input_id in zip(roles, input_ids):
        ...
        if role == 'system' or role == 'human':
            ...
            answer_first_index += input_id.shape[0]
        elif role == 'gpt':
            ...
            answer_first_index+=ignore_len
            answer_last_index = answer_first_index + input_id.shape[0] - ignore_len - 3
        else:
            raise NotImplementedError
    ...
    classify_info = {'answer_tokens': answer_tokens, 'choices_ids': choices_ids, 'answer_first_index': answer_first_index, 'answer_last_index': answer_last_index} if answer_tokens is not None else None
    return dict(
        input_ids=input_ids,
        labels=targets,
        attention_mask=input_ids.ne(tokenizer.pad_token_id),
